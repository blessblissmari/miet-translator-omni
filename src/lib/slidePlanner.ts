...
import { economyVisionModel } from "./layoutBlueprint";
...
async function planSlideRobust(
  ...
  const bestImg = page.imageDataUrl || (realImages.length > 0
  ...
  > = [
    {
      type: "text",
      text: `Slide/page ${page.index + 1}. Treat PPTX like a PDF page: the attached rendered slide image is authoritative. Translate visible academic content to Russian. Use this extracted text only as a weak hint:\n\n${page.text.slice(0, 6000)}`,
    },
  ];
  if (page.imageDataUrl) {
    userContent.push({ type: "image_url", image_url: { url: await downsampleDataUrl(page.imageDataUrl, { maxDim: 1600 }) } });
  }
  ...
  try {
    ...
    if (bestImg && layout === "title-text") layout = "title-text-image-right";
    if (!bestImg && (layout === "title-text-image-right" || layout === "title-text-image-left" || layout === "title-image")) layout = "title-text";
    return {
    ...
    const plain = await chat({
      ...
      messages: [
        {
          role: "system",
          content: `Translate the rendered academic slide into ${TARGET_LANG}. Output a short ${TARGET_LANG} title on the first line, then up to 8 bullet lines starting with "- ". Keep math in $...$ or $$...$$. No commentary.`,
        },
        { role: "user", content: userContent },
      ],
    });
