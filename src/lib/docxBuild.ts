import {
  ...
  Table, TableRow, TableCell, WidthType, BorderStyle, LevelFormat, TableLayoutType, VerticalAlignTable,
} from "docx";
...
function textRunsWithRichText(text: string, opts: { bold?: boolean } = {}): TextRun[] {
  const runs: TextRun[] = [];
  const re = /<\/?(sub|sup)>/gi;
  let pos = 0;
  ...
  while ((m = re.exec(text))) {
    ...
    if (seg) runs.push(new TextRun({ text: seg, bold: opts.bold, subScript: mode === "sub", superScript: mode === "sup" }));
    const closing = m[0].startsWith("</");
    const tag = m[1].toLowerCase();
    mode = closing ? "normal" : (tag === "sub" ? "sub" : "sup");
    pos = re.lastIndex;
  }
  const rest = text.slice(pos);
  if (rest) runs.push(new TextRun({ text: rest, bold: opts.bold, subScript: mode === "sub", superScript: mode === "sup" }));
  return runs;
}

/** Split a text run by $...$ and $$...$$ markers and emit TextRuns interleaved with formula-marker placeholders. */
function runsWithMath(text: string, formulas: FormulaMarker[], opts: { bold?: boolean } = {}): TextRun[] {
  ...
  while ((m = re.exec(text))) {
    if (m.index > last) {
      ...
      if (seg) out.push(...textRunsWithRichText(seg, opts));
    }
    const isDisplay = !!m[1];
  ...
  if (last < text.length) out.push(...textRunsWithRichText(text.slice(last), opts));
...
async function blockToElements(block: DocBlock, formulas: FormulaMarker[]): Promise<(Paragraph | Table)[]> {
  switch (block.type) {
    case "table": {
      const maxCols = Math.max(1, ...block.rows.map((r) => r.length));
      ...
      const rows = block.rows.map((row, ri) => new TableRow({
        children: row.map(cell => new TableCell({
          ...
          children: [new Paragraph({
            alignment: ri === 0 && block.header ? AlignmentType.CENTER : AlignmentType.LEFT,
    ...
    case "para":
      return [new Paragraph({ alignment: AlignmentType.JUSTIFIED, children: runsWithMath(block.text, formulas) })];
    case "list":
