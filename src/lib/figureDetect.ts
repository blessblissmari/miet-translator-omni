...
const DETECT_PROMPT = `You are a layout analyzer for an academic translation pipeline.

Given the page image, return ONLY a strict JSON array of every visual element that must survive in the translated output:
- figures, diagrams, schematics, circuits
- charts, plots, graphs
- photographs, screenshots
- tables that are better preserved as a visual image
- full-page slide graphics when the page is a presentation-like page
...
Each element MUST have a tight bounding box around ONLY the figure itself, NOT the surrounding body text. Include labels/axes/legends for charts. Caption text goes in the "caption" field if adjacent.
Prefer one complete chart/diagram bbox over many tiny bboxes. The downstream cropper will render on a white background.

Output format (JSON only, no markdown fences, no commentary):
[{"x":0.12,"y":0.34,"w":0.5,"h":0.22,"kind":"chart","caption":"Fig. 3 — signal flow"}]
...
function parseBBoxes(s: string): FigureBBox[] {
  ...
  for (const it of arr) {
    ...
    if (w >= 0.99 && h >= 0.99) continue; // ignore exact full-page (likely the page itself)
    out.push({
